...
Final URL: https://featkit.io/p/[slug]?token=[base64payload].[signature]
```
...
def generate_signed_link(slug, secret_key, user_id, email=None, name=None):
    ...
    return f"https://featkit.io/p/{slug}?token={payload_b64}.{signature}"
```
...
function generateSignedLink(slug, secretKey, userId, email = null, name = null) {
  ...
  return `https://featkit.io/p/${slug}?token=${payloadB64}.${signature}`;
}
```
...
def generate_signed_link(slug, secret_key, user_id, email: nil, name: nil)
  ...
  "https://featkit.io/p/#{slug}?token=#{payload_b64}.#{signature}"
end
```
</CodeGroup>
